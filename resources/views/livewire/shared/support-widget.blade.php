<div class="fixed bottom-5 left-5 z-40 w-[min(22rem,calc(100vw-2rem))]">
    <div
        ...
        @class([
            'support-widget__panel surface-card mb-3 rounded-[1.5rem] p-5 shadow-[0_20px_50px_rgba(15,23,42,0.1)]',
            'hidden' => ! $open,
        ])
        role="region"
    ...
    >
        @if ($open)
            <p class="section-kicker">{{ $support['title'] }}</p>
    ...
    <button
        type="button"
        wire:click="toggle"
        class="support-widget__toggle btn-primary w-full justify-between !rounded-[1.1rem] !px-5"
